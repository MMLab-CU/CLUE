...
.. note::
    ...
    provided as global functions (within the namespace ``clue``) instead of
    member functions.
...
The following code snippet uses the functionalities provided in *CLUE++* to
...
- ``starts_with``, which checks whether a string starts with a certain
  character of sub-string. *CLUE++* also provides ``ends_with`` to check
...
Below, we introduce other string-related functionalities provided by *CLUE++*.
